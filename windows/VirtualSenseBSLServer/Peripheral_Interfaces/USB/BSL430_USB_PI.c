...
#define USB_PI 0x30
#define USB_PI_VERSION 0x05
#define PI_VERSION (USB_PI+USB_PI_VERSION) // 0x35
...
#define ACLK_DIV_SPEED  (ACLK_SPEED/4)
  ...
  if( TIMER_CCR > (((SPEED_1 + SPEED_2) / 2)/ ACLK_DIV_SPEED) )            
  {
  ...
  else if( TIMER_CCR > (((SPEED_2 + SPEED_3) / 2)/ ACLK_DIV_SPEED) )
  {
    wUSBPLL = *(&cSPEED_2_PLL);
  }
  else if( TIMER_CCR > (((SPEED_3 + SPEED_4) / 2)/ ACLK_DIV_SPEED) )
  {  
