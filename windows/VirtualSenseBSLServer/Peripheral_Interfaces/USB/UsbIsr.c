...
|                                                                             |
+-----------------------------------------------------------------------------+
|  Source: UsbIsr.h, v1.4 2009/10/13                                          |
...
#endif
{
    ...
    if (USBIFG & SETUPIFG)
    {
        ...
        //return;  no return - to save code space!
    } 
    
    i = USBVECINT;    // read out the interrupt vector register
    
    if (i == VECINT_PWR_VBUSOn_INTERRUPT)
      PWRVBUSonHandler();
    else if (i == VECINT_PWR_VBUSOff_INTERRUPT)
      PWRVBUSoffHandler();
    else if (i == VECINT_INPUT_ENDPOINT0)
      IEP0InterruptHandler();
    /*  else if (i == VECINT_OUTPUT_ENDPOINT0)
      //OEP0InterruptHandler(); */
    else if (i == VECINT_RSTR_INTERRUPT)
      USB_reset();
    else if (i == VECINT_SUSR_INTERRUPT)
      USB_suspend();
    else if (i == VECINT_RESR_INTERRUPT)
      USB_resume();
    
    //work around: no setup handling here, it needs higher priority 
    //and will be performed above.
    //
    //else if (i == VECINT_SETUP_PACKET_RECEIVED)
    //  SetupPacketInterruptHandler();  
}
...
VOID SetupPacketInterruptHandler(VOID)
{
    ...
    //tEndPoint0DescriptorBlock.bIEPBCNT = EPBCNT_NAK;    //-- - the bit should be set by the HW(workaround Rosty)
...
{
//--      tEndPoint0DescriptorBlock.bIEPBCNT = EPBCNT_NAK; // NAK IEP0 during filling buffer; maybe not requiered, just in case (workaround for IEP0 NAK bug)
    ...
    if(bStatusAction == STATUS_ACTION_DATA_OUT)
    {
        ...
        if(bStatusAction == STATUS_ACTION_NOTHING)
        {
            if ((tSetupPacket.bRequest == USB_REQ_SET_REPORT) &&
             (tSetupPacket.bValueH == USB_REQ_HID_OUTPUT))
            {
                Handler_OutReport();
            }
        }  * /
